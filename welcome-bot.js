...
const config = {
  ...
  twitchBannedWords:  [],   // Mots bannis sur Twitch aussi
...
const QCM = [
  ...
  { question: "❓ Question 3 — Que risques-tu si tu ne respectes pas les règles ?",
    answers: [{ label: "Un warn, mute ou ban", correct: true }, { label: "Rien du tout", correct: false }, { label: "Un message privé sympa", correct: false }] },
];
...
async function checkTwitchStream() {
  ...
  try {
    ...
    const data = await httpsGet(
      `https://api.twitch.tv/helix/streams?user_login=${config.twitchUsername}`,
      { "Client-ID": config.twitchClientId, "Authorization": `Bearer ${twitchAccessToken}` }
    );
  ...
  } catch (e) { twitchAccessToken = null; console.error("⚠️ Erreur Twitch :", e.message); }
}
...
async function announceStream(stream) {
  try {
    ...
    const embed = new EmbedBuilder().setColor("#9146ff")
      ...
      .setURL(`https://twitch.tv/${config.twitchUsername}`)
      .setFooter({ text: "Tom_O_Carre • Twitch" }).setTimestamp();
    if (config.showThumbnail) embed.setImage(thumbnail);
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setLabel("🎮 Regarder le live").setURL(`https://twitch.tv/${config.twitchUsername}`).setStyle(ButtonStyle.Link)
    );
    const mention = config.mentionEveryone ? "@everyone 🔴 **Le stream vient de commencer !**" : "🔴 **Le stream vient de commencer !**";
    await liveChannel.send({ content: mention, embeds: [embed], components: [row] });
  ...
  } catch (e) { console.error("⚠️ Erreur annonce live :", e.message); }
}

// ════════════════════════════════════════════════
//  TWITCH CHAT (test de message)
// ════════════════════════════════════════════════
async function sendTwitchChatMessage(message) {
  try {
    ...
    const userData = await httpsGet(
      `https://api.twitch.tv/helix/users?login=${config.twitchUsername}`,
      { "Client-ID": config.twitchClientId, "Authorization": `Bearer ${twitchAccessToken}` }
    );
    if (!userData.data || !userData.data.length) throw new Error("Utilisateur Twitch introuvable");
    const broadcasterId = userData.data[0].id;

    // Envoie le message
    const result = await new Promise((resolve, reject) => {
      const body = JSON.stringify({ broadcaster_id: broadcasterId, sender_id: broadcasterId, message });
      const req = https.request("https://api.twitch.tv/helix/chat/messages", {
        method: "POST",
        headers: {
    ...
    dashData.twitchChatLogs.push({ message, time: new Date().toLocaleString("fr-FR"), status: result.status === 200 ? "✅" : "❌" });
    return result.status === 200;
  } catch (e) {
    console.error("⚠️ Erreur chat Twitch :", e.message);
    return false;
  }
}
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url === "/api/data") {
      ...
      return res.end(JSON.stringify({
        ...
        isLive: isStreamLive, config: {
          twitchUsername: config.twitchUsername, twitchClientId: config.twitchClientId,
          announceEnabled: config.announceEnabled, mentionEveryone: config.mentionEveryone,
          showThumbnail: config.showThumbnail, twitchChatModEnabled: config.twitchChatModEnabled,
        }
      }));
    }

    // ── POST /api/settings ── (mise à jour token Discord, config Twitch...)
    ...
    // ── POST /api/twitch/test ── (envoyer un message test dans le chat Twitch)
    if (url === "/api/twitch/test" && req.method === "POST") {
      let body = ""; req.on("data", d => body += d);
      req.on("end", async () => {
        try {
          const { message } = JSON.parse(body);
          const ok = await sendTwitchChatMessage(message || "👋 Test depuis le dashboard Tom_O_Carre !");
          res.writeHead(200, { "Content-Type": "application/json" });
  ...
  try {
    ...
    if (rulesChannel) {
      ...
      if (alreadyPosted) {
        ...
        const embed = new EmbedBuilder().setColor("#5865f2")
          .setTitle("📜 Règles du serveur Tom_O_Carre").setDescription(RULES)
          .setFooter({ text: "Lis bien les règles avant de participer !" });
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId("start_qcm").setLabel("✅ J'ai lu les règles — Passer le QCM").setStyle(ButtonStyle.Success)
        );
...
client.on("messageCreate", async (message) => {
  ...
  if (banned) {
    try {
      ...
      await message.author.send(`⚠️ **Ton message sur le serveur Tom_O_Carre a été supprimé.**\nIl contenait un mot interdit. Merci de respecter les règles ! 🙏`).catch(() => {});
      ...
      const logsChannel = message.guild.channels.cache.find(c => c.name === LOGS_CHANNEL);
      if (logsChannel) {
        await logsChannel.send({ embeds: [new EmbedBuilder().setColor("#ed4245").setTitle("🚫 Message supprimé — Mot interdit")
          .addFields({ name: "Membre", value: `<@${message.author.id}> (${message.author.tag})`, inline: true },
                     { name: "Salon", value: `<#${message.channel.id}>`, inline: true },
                     { name: "Mot", value: `\`${banned}\``, inline: true },
                     { name: "Message", value: `\`\`\`${message.content.slice(0, 300)}\`\`\`` }).setTimestamp()] });
      }
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId.startsWith(`qcm_${userId}_`)) {
    ...
    if (chosen.correct) {
      ...
      sessions.delete(userId);
      await interaction.update({ embeds: [new EmbedBuilder().setColor("#ed4245").setTitle("❌ Mauvaise réponse !").setDescription("Relis bien les règles et réessaie en cliquant sur le bouton ✅.")], components: [] });
    }
  }
});
